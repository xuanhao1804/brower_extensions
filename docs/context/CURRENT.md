...
- **Latest product implementation:** Store-ready version `0.1.3` packages the Facebook Reel placement fix: the controller anchors at the rendered video's bottom-right with 18px bottom clearance, and expanded controls grow leftward; other sites retain their existing placement.
...
- **GitHub synchronization:** Facebook Reel placement fix `eefa8b7` and `0.1.3` package version commit `320ca41` are pushed; #20 is closed. The earlier Speedable rename remains tracked by open Issue #19 for public Store verification. Historical Issues #2–#14 remain available for earlier work.
...
- **Verification baseline:** On 2026-09-24, `npm run check`, `npm run build` (Chrome MV3), `npm run build:firefox` (Firefox MV2), and `npm run zip` passed for `0.1.3`. The generated Chrome manifest reports version `0.1.3`, unchanged permissions and the existing Firefox ID; ZIP contents and the built Facebook route guard were inspected. Direct unpacked-extension checks on Facebook's dynamic UI remain recommended.
- **Next recommended action:** Upload `.output/speedable-0.1.3-chrome.zip` to the existing Chrome Web Store item, update the detailed Store description to use the Speedable brand, change the support URL to `https://github.com/xuanhao1804/brower_extensions/issues`, and verify the public item displays `0.1.3` after approval. Separately finish promotional assets and verify Edge Add-ons visibility.
